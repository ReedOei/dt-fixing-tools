...
QUERY="select s.slug, ftc.subject_name, ftc.test_name, sr.sha from fixable_tests_fmo ftc join subject s on ftc.subject_name = s.name join subject_raw sr on sr.slug = s.slug;"
